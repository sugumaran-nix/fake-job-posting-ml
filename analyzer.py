...
# Maps a brand keyword → official domain.
# Used for typosquatting detection in both URL and company name checks.
BRAND_DOMAINS = {
    "infosys":   "infosys.com",
    "tcs":       "tcs.com",
    "wipro":     "wipro.com",
...
def analyse_company(company: str) -> dict:
    ...
    # 1. Known legitimate company — reduces score but does NOT block
    #    subsequent scam-keyword or brand-impersonation checks.
    #    A real brand name can still appear alongside scam words
    #    (e.g. "Infosys Earn Daily Work From Home").
    ...
    # 3. Brand impersonation — always checked, regardless of is_known.
    #    FIX: original code used `if brand in cl and not is_known`, which
    #    meant a name like "Infosys Earn Daily" was never flagged for
    #    impersonation because is_known was set True in step 1.
    #    Now: flag whenever the brand appears AND the name has additional
    #    words that change the identity (word count > 3 or scam words present).
    ...
    vague_patterns = [
        (r"^(a\s+)?(company|firm|organization|agency|enterprise)s?$",
         "Generic placeholder name"),
        (r"^[A-Z]{1,4}$",
         "Only initials — no real identity"),
    ...
    # 5. Excessive legal / geographic suffixes stacked
    scount = len(re.findall(
        r"\b(pvt|ltd|llp|inc|corp|llc|international|global|india|worldwide|enterprises|group)\b",
        cl,
    ))
    if scount >= 3:
    ...
    # ── Risk calculation ───────────────────────────────────────
    # FIX: If scam keywords were matched, floor risk at "medium"
    # regardless of the known-brand score reduction in step 1.
    # This prevents "Infosys Earn Daily" resolving to "low" risk.
